...
class Mail{
	...
	public function setFrom($str){
		...
		$this->headers = 'From:' . $this->from . PHP_EOL . 'X-Mailer: PHP/' . phpversion() . PHP_EOL; 
	}
	...
	public function setOrder($arr){
		$this->appendOrder = Regex::getArray($arr);
	}
	...
	public function hasNone($arr){
		foreach($arr as $key => $option){
			$this->addHasNone($key, $option);
		}
	}
	
	private function addHasAny($field, $types){		
		$this->hasAny[] = array("field" => $field,
					"types" => $types,
					"value" => $this->postArr[$field]);
	}
	
	private function addHasNone($field, $types){	
	...
	public function addHeader($field, $value){
		$this->headers .= "$field: $value" . PHP_EOL;
	}

	public function addAttacment($filename, $attachment){
		$eol = PHP_EOL;
		$separator = md5(time());
		...
		$this->headers .= $attachment.$eol.$eol;
